...
* ## Intro(汇编语言(第4版)王爽-阅读笔记)
    ...
    + ### 附注4 用栈传递参数
        ...
        <br><br>从汇编代码中可以分析得知：
        <br><span style='padding-left:2.3em'>`1)`、有些局部变量用寄存器接收（a->di，c->si），有些临时存在栈里面了（b->stack），但是最终所有的函数参数都是通过复制入栈传递。
        ...
        <!-- tabs:end -->
        <!-- panels:end -->
        ![](/.images/corner/assembly/book/as-ws/book-anno-4-03.png ':size=99%')