...
* ## Intro(GIT | git | vcs | 版本控制系统)
	...
	* ### 网络抖动
		...
		> [?] git clone 中断后要从头开始下载，很让人无奈。这通常是因为网络不稳定，抖动导致下载中断，且 Git 默认不支持像普通下载工具那样的“断点续传”。
		<br><br>解决方式如下：
		<br>1、浅克隆 `git clone --depth 1 https://github.com/google/bindiff.git`，后续想补全可以：`git fetch --unshallow`。[--unshallow](https://git-scm.com/docs/git-fetch#Documentation/git-fetch.txt---unshallow)
		<br>2、手动创建仓库初始化并关联远程仓库，然后逐步拉取，比如`git fetch origin 1`，`git fetch origin 20`，`git fetch origin end`等。补全`git fetch --unshallow`

		git fetch 内部以某种形式（commit，objects...）为单位，在这个单位上进行增量获取，不像 http 断点续传中以字节为单位。简单翻译过来就是，fetch 向服务器发送请求的时候，我目前有哪些单位，缺哪些，然后服务器将**差值**全量打包传送，这个过程中有任何数据丢失，已经传送的也会丢弃。
		
		最直接的表现就是初次 clone 一个大仓库，遇到网络中断，都得从头来获取所有差值，而正常使用中获取最近提交都特别快（差值少）。
	...
	* ### GITHUB 获取第一次 commit
		...
		> [?] 查看当前分支 main 提交总数: **\$commits**，然后查看最新 commit: **\$last_hash** 通过 `https://github.com/xxx/xxx/commits/main/?after= $last_hash + $(commits - 1)`，注意 url 不要有空格。
